...
public class SawWaveAudio extends IAudioStream {
	private double startTime_;
	private double freq_;
	private double time_;
	private int currentFrame_;

	public SawWaveAudio(double freq, double startTime, double time, MMLParser.Note note) {
		freq_ = freq;
	...
	public int readFloat(float[] data, int offset, int frameN) {
		...
		for (int i = 0; i < frameN; i++) {
			...
			double p = (t % (1 / freq_))  * 2048 * freq_ - 1024;
